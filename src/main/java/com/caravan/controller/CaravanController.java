...
public class CaravanController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Caravan> updateCaravan(@PathVariable Long id, @Valid @RequestBody Caravan caravan) {
        try {
    ...
    public ResponseEntity<List<Caravan>> getCaravansByPriceRange(
        ...
        return ResponseEntity.ok(caravanService.getCaravansByPriceRange(min, max));
    }

    @GetMapping("/bunk-beds/{minBunkBeds}")
    public ResponseEntity<List<Caravan>> getCaravansByMinimumBunkBeds(@PathVariable Integer minBunkBeds) {
        return ResponseEntity.ok(caravanService.getCaravansByMinimumBunkBeds(minBunkBeds));
    }
}
